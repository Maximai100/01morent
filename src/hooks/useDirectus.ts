...
// Хук для миграции данных из Supabase в Directus
export const useDataMigration = () => {
  ...
  const migrateFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      const { dataUtils } = await import('@/integrations/directus/client');
      ...
      if (apartments) {
        for (let i = 0; i < apartments.length; i++) {
          ...
          setProgress((i + 1) / apartments.length * 50);
        }
      }
      ...
      if (guests) {
        for (let i = 0; i < guests.length; i++) {
          ...
          const directusBooking = dataUtils.convertGuestToDirectus(guest);
          await bookingsAPI.create(directusBooking);
          setProgress(50 + (i + 1) / guests.length * 50);
        }
      }

      toast.success('Миграция данных завершена успешно');
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Ошибка миграции данных';
      toast.error(errorMessage);
      throw err;
    } finally {
      setMigrating(false);
